...
class BaseImage:
	...
	def to_matrix(self, data):
		...
		while i < len(data):
			...
			row = list()
			while x < self.w:
				r = self.to_256(data[i])
				i += 1
				g = self.to_256(data[i])
				i += 1
				b = self.to_256(data[i])
				i += 1
	...
	def to_data(self):
		...
		for row in self.pixels:
			for pixel in row:
				data.append(self.from_256(pixel.r))
				data.append(self.from_256(pixel.g))
				data.append(self.from_256(pixel.b))
				if self.has_alpha:
					data.append(pixel.a)
		return data
